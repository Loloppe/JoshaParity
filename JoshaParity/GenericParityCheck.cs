    ...
    public class GenericParityCheck : IParityMethod
    {
        ...
        public Parity ParityCheck(ref SwingData currentSwing, ParityCheckContext context)
        {
            ...
            for (int i = 0; i < intervalGrids.Count; i++)
            {
                ...
                int cutDirT = lastSwing.notes.All(x => x.CutDirection == 8) ?
                ...
                if (currentSwing.notes.All(x => x.CutDirection == 8))
                {
                ...
                if (currentSwing.notes.Any(x => x.CutDirection != 8))
                {
                    // As a rule of thumb:
                    // If the rotation is bigger when the next swing is forehand, we go backhand, and vice versa

                    // Calculate AFN values
                    float forehandAFN = ParityUtils.ForehandDict(rightHand)[currentSwing.notes.First(x => x.CutDirection != 8).CutDirection];
                    float backhandAFN = ParityUtils.BackhandDict(rightHand)[currentSwing.notes.First(x => x.CutDirection != 8).CutDirection];
